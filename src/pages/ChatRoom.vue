...
onMounted(async () => {
  ...
  console.log(
    "채팅방 상태:",
    isCompleted.value,
    isCompleted.value === "true" ? "완료" : "진행 중"
  );
  console.log("상대방 ID:", othersId, typeof othersId);
  console.log("구매를 한 사람인지: ", isBuyer.value);
  console.log("review button", isCompleted.value && isBuyer.value);
  ...
  if (!isBuyer.value) {
    ...
    getUserDetail(buyerId)
      .then((data) => {
        ...
        isRestricted.value = isRestricted.value || !data.data.data.isActive; // 구매자 제한 여부
      })
      .catch((error) => {
